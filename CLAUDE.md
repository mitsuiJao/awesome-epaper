...
- **`src/waveshare_epd/`** — vendored Waveshare driver for the display (`epd7in5b_V2.py` + `epdconfig.py`), talking SPI/GPIO via a `RaspberryPi` config class. This is the known-good, working driver — do not modify it. It stays directly under `src/` because it's a shared module imported by both `src/server/lib/epd_backend.py` (lazily, only in real-hardware mode — see below) and `src/server/epd_7in5b_V2_test.py`.
- **`src/server/epd_7in5b_V2_test.py`** — Waveshare's stock hardware smoke-test demo (draws sample text/shapes, then a bmp, then clears). Used to verify the HAT is wired correctly. The demo body is wrapped in a `run_demo()` function so `main.py` can call it directly (see `POST /draw/test` below); the drawing sequence itself is unchanged and still treated as the known-good reference — only the entry point was changed to make it importable. The `waveshare_epd` import is lazy (inside `run_demo()`, not at module level) so importing this file never touches hardware detection unless `run_demo()` actually runs. Standalone invocation (`if __name__ == '__main__'`) still resolves `waveshare_epd` via `sys.path[0]` (its own directory, `src/server/`) plus `PYTHONPATH=src` at run time, since the driver lives one level up — see README for the exact command.
